...
class TestHasText:
    """Tests for pdf.has_text."""

    @patch("paperless_pre_consume_ocr.pdf.extract_text")
...
class TestGetMetadata:
    """Tests for pdf.get_metadata."""

    def test_nonexistent_file_returns_none(self, tmp_path):
        """Should return None for non-existent file."""
        result = pdf.get_metadata(tmp_path / "nonexistent.pdf")
    ...
    def test_non_pdf_file_returns_none(self, tmp_path):
        ...
        txt_file.touch()
        result = pdf.get_metadata(txt_file)
        assert result is None

    @patch("paperless_pre_consume_ocr.pdf.pikepdf")
    def test_metadata_extracted(self, mock_pikepdf, tmp_path):
        ...
        result = pdf.get_metadata(pdf_file)
...
class TestCheckMetadataPattern:
    """Tests for pdf.check_metadata_pattern."""

    @patch("paperless_pre_consume_ocr.pdf.get_metadata")
    def test_pattern_found(self, mock_get_metadata):
        ...
        assert pdf.check_metadata_pattern(Path("/tmp/test.pdf"), r"Tesseract|ocrmypdf") is True
    ...
    def test_pattern_not_found(self, mock_get_metadata):
        ...
        assert pdf.check_metadata_pattern(Path("/tmp/test.pdf"), r"Tesseract|ocrmypdf") is False

    @patch("paperless_pre_consume_ocr.pdf.get_metadata")
    def test_no_metadata_returns_false(self, mock_get_metadata):
        """Should return False when metadata is None."""
        mock_get_metadata.return_value = None
        assert pdf.check_metadata_pattern(Path("/tmp/test.pdf"), r"Tesseract") is False
